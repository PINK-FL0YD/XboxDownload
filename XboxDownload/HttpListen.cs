        ...
        private void TcpThread(object obj)
        {
            ...
            if (mySocket.Connected)
            {
                ...
                while (mySocket.Connected && mySocket.Poll(3000000, SelectMode.SelectRead))
                {
                    ...
                    Match result = Regex.Match(_buffer, @"(?<method>GET|OPTIONS) (?<path>[^\s]+)");
                    if (!result.Success)
                    {
                        mySocket.Close();
                        continue;
                    ...
                    else
                    {
                        ...
                        else
                        {
                            ...
                            if (_domainName == "dl.delivery.mp.microsoft.com" || _extension == ".phf" || _extension == ".json") //代理 Xbox|PS 下载索引
                            {
                                ...
                                if (!string.IsNullOrEmpty(ip))
                                {
                                    ...
                                    if (string.IsNullOrEmpty(socketPackage.Err))
                                    {
                                        ...
                                        if (Form1.bRecordLog)
                                        {
                                            ...
                                            if (_domainName.EndsWith(".prod.dl.playstation.net") && _extension == ".json") //分析PS游戏下载地址
                                            {
